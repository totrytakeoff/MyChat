...
- User Service (Phase 4) provides register/login/profile via ODB-backed PostgreSQL persistence, using PBKDF2-HMAC-SHA256 password hashing. The User gRPC path is available behind `MYCHAT_BUILD_USER_GRPC_SERVICE=ON`: `im.user.UserService.Register/Login/GetUserInfo` maps to the same core `UserService` semantics through `services/user/UserGrpcService`; `services/user/user_server` hosts that adapter as a standalone process on `user.listen_address` (default `0.0.0.0:9001`); Gateway can route existing User HTTP handlers through `RemoteUserClient` when `user.mode=remote`.
...
- Message Service persistence core (Task 003) provides ODB-backed `im_messages` storage, validated one-to-one text send, chronological conversation history, offline pull for undelivered messages, and delivered/read marking. The Message gRPC path is available behind `MYCHAT_BUILD_MESSAGE_GRPC_SERVICE=ON`: `im.message.MessageService.SendMessage/GetConversation/PullOffline/MarkDelivered/MarkRead` maps to the same core `MessageService` semantics through `services/message/MessageGrpcService`; `services/message/message_server` hosts that adapter as a standalone process on `message.listen_address` (default `0.0.0.0:9002`). Gateway can route existing Message HTTP, Message WS persistence, and Push delivered marking through `RemoteMessageClient` when `message.mode=remote`, while `message.mode=local` remains the default.
...
- CMake staged build options: `MYCHAT_BUILD_TESTS`, `MYCHAT_BUILD_GATEWAY`, `MYCHAT_BUILD_SERVICES`, `MYCHAT_BUILD_USER_SERVICE`, `MYCHAT_BUILD_USER_GRPC_SERVICE`, `MYCHAT_BUILD_MESSAGE_SERVICE`, `MYCHAT_BUILD_MESSAGE_GRPC_SERVICE`, `MYCHAT_BUILD_FRIEND_SERVICE`, `MYCHAT_BUILD_FRIEND_GRPC_SERVICE`, `MYCHAT_BUILD_GROUP_SERVICE`, `MYCHAT_BUILD_GROUP_GRPC_SERVICE`, `MYCHAT_BUILD_CODEC_SERVICE`, `MYCHAT_BUILD_PGSQL_ODB`, `MYCHAT_BUILD_LEGACY_GATEWAY_TESTS`, `MYCHAT_BUILD_LEGACY_UNIT_TESTS`. Explicit gRPC service options require the `codec-grpc` vcpkg feature; `MYCHAT_BUILD_USER_GRPC_SERVICE=ON` builds both `im::user_grpc_service` and the `user_server` executable, `MYCHAT_BUILD_MESSAGE_GRPC_SERVICE=ON` builds both `im::message_grpc_service` and the `message_server` executable, `MYCHAT_BUILD_FRIEND_GRPC_SERVICE=ON` builds both `im::friend_grpc_service` and the `friend_server` executable, and `MYCHAT_BUILD_GROUP_GRPC_SERVICE=ON` builds both `im::group_grpc_service` and the `group_server` executable.
- Friend Service MVP provides friend request/respond/list/pending flows with ODB persistence and Gateway HTTP routes. The Friend gRPC path is available behind `MYCHAT_BUILD_FRIEND_GRPC_SERVICE=ON`: `im.friend_.FriendService.SendRequest/RespondToRequest/GetFriends/GetPendingRequests` maps to the same core `FriendService` semantics through `services/friend/FriendGrpcService`; `services/friend/friend_server` hosts that adapter as a standalone process on `friend.listen_address` (default `0.0.0.0:9003`). Gateway can route existing Friend HTTP handlers through `RemoteFriendClient` when `friend.mode=remote`, while `friend.mode=local` remains the default. The proto package is `im.friend_` because `friend` is a C++ keyword and gRPC C++ generation does not escape it safely.
- Group Service MVP provides group create/join/leave/list/member flows, group message persistence/history, and multi-recipient fanout through `PushService::push_to_user`. The Group gRPC path is available behind `MYCHAT_BUILD_GROUP_GRPC_SERVICE=ON`: `im.group.GroupService.CreateGroup/JoinGroup/LeaveGroup/ListMyGroups/ListMembers/SendGroupMessage/GetGroupMessages` maps to the same core `GroupService` and `GroupMessageService` semantics through `services/group/GroupGrpcService`; `services/group/group_server` hosts that adapter as a standalone process on `group.listen_address` (default `0.0.0.0:9004`). Gateway remote Group facades are still the next slice; `group.mode=local` remains the only Gateway path today.
...
- User Service remote process/client work now provides `services/user/user_server`, `Gateway` `UserClient` local/remote facade, `RemoteUserClient`, `UserServerAppTest`, `RemoteUserClientTest`, and `RemoteUserGatewayServerSmokeTest`. Gateway defaults to local in-process UserService via `user.mode = "local"` and preserves the existing HTTP auth/profile contract when `user.mode=remote`.
- Active focused ODB + Gateway + Push/User/Message/Friend/Group gRPC regression on 2026-06-07: `GroupServiceCoreTest`, `GroupGrpcServiceTest`, and `GroupServerAppTest` passed in `build/remote-push-odb` with `MYCHAT_BUILD_GROUP_GRPC_SERVICE=ON`. A separate gRPC-default-off path passed `GroupServiceCoreTest`, `GatewayGroupHttpTest`, and `GatewayGroupMessageHttpTest` in `build/group-grpc-off-verify`. Friend remote verification previously passed `FriendServiceCoreTest`, `FriendGrpcServiceTest`, `FriendServerAppTest`, `GatewayFriendHttpTest`, `RemoteFriendClientTest`, and `RemoteFriendGatewayServerSmokeTest`.
...
- Redis wrapper now uses a small RAII connection pool keyed by
  ...
  RedisClient reconnects and retries once after hiredis reports a
  connection-level empty reply; focused Redis coverage kills a live Redis
  client connection and verifies the next command succeeds.
