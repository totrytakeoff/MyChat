...
- CMake staged build options: `MYCHAT_BUILD_TESTS`, `MYCHAT_BUILD_GATEWAY`, `MYCHAT_BUILD_SERVICES`, `MYCHAT_BUILD_USER_SERVICE`, `MYCHAT_BUILD_USER_GRPC_SERVICE`, `MYCHAT_BUILD_MESSAGE_SERVICE`, `MYCHAT_BUILD_MESSAGE_GRPC_SERVICE`, `MYCHAT_BUILD_FRIEND_SERVICE`, `MYCHAT_BUILD_FRIEND_GRPC_SERVICE`, `MYCHAT_BUILD_CODEC_SERVICE`, `MYCHAT_BUILD_PGSQL_ODB`, `MYCHAT_BUILD_LEGACY_GATEWAY_TESTS`, `MYCHAT_BUILD_LEGACY_UNIT_TESTS`. Explicit gRPC service options require the `codec-grpc` vcpkg feature; `MYCHAT_BUILD_USER_GRPC_SERVICE=ON` builds both `im::user_grpc_service` and the `user_server` executable, `MYCHAT_BUILD_MESSAGE_GRPC_SERVICE=ON` builds both `im::message_grpc_service` and the `message_server` executable, and `MYCHAT_BUILD_FRIEND_GRPC_SERVICE=ON` builds both `im::friend_grpc_service` and the `friend_server` executable.
...
- Active focused ODB + Gateway + Push/User/Message/Friend gRPC regression on 2026-06-07: `FriendServiceCoreTest`, `FriendGrpcServiceTest`, `FriendServerAppTest`, `GatewayFriendHttpTest`, `RemoteFriendClientTest`, and `RemoteFriendGatewayServerSmokeTest` passed in `build/remote-push-odb` with `MYCHAT_BUILD_FRIEND_GRPC_SERVICE=ON`. A separate gRPC-default-off path passed `FriendServiceCoreTest` and `GatewayFriendHttpTest` in `build/friend-grpc-off-verify`. Earlier User/Message/Push remote regressions passed in `build/remote-push-odb` on 2026-06-06.
