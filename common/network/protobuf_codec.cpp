...
bool ProtobufCodec::encode(const im::base::IMHeader& header,
    ...
    try {
        ...
        LogManager::GetLogger("protobuf_codec")
                ->debug("Header size: {}, Type name: {}, Message data size: {}", header_size,
                        message_type, message_data.size());
...
bool ProtobufCodec::decode(const std::string& input,
    ...
    try {
        ...
        ArrayInputStream array_in(data_without_crc.data(),
                                  static_cast<int>(data_without_crc.size()));
        ...
        if (type_name_size == 0) {
            LogManager::GetLogger("protobuf_codec")
                    ->error("Invalid type name size: {}", type_name_size);
            return false;
        }
        ...
        int remaining_bytes =
                static_cast<int>(data_without_crc.size()) - after_header_and_type_sizes_pos;
        ...
        // 6. 读取消息类型名称
        std::string type_name_data(data_without_crc.data() + after_header_and_type_sizes_pos,
                                   type_name_size);
        ...
        // 7. 读取消息头数据
        std::string header_data(
                data_without_crc.data() + after_header_and_type_sizes_pos + type_name_size,
                header_size);
        ...
        LogManager::GetLogger("protobuf_codec")
                ->debug("Message position: {}, Message size: {}", message_position, message_size);
...
base::IMHeader ProtobufCodec::returnHeaderBuilder(base::IMHeader header, std::string device_id,
                                                  std::string platform) {
    base::IMHeader returnHeader;
    ...
    // 2. 转换为自纪元以来的毫秒数
    auto milliseconds = duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
    // 3. 获取数值
    auto ms_count = milliseconds.count();

    returnHeader.set_timestamp(ms_count);
    returnHeader.set_token(header.token());
    returnHeader.set_device_id(device_id);
