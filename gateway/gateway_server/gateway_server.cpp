...
GatewayServer::GatewayServer(const std::string platform_strategy_config,
    ...
    // 执行完整的服务器组件初始化；失败时不能留下半初始化的可启动对象。
    if (!init_server(ws_port, http_port)) {
        throw std::runtime_error("Failed to initialize GatewayServer");
    }
}
...
void GatewayServer::start() {
    ...
    try {
        ...
        if (!websocket_server_ || !http_server_) {
            throw std::runtime_error("GatewayServer network components are not initialized");
        }
        ...
        if (http_thread_.joinable()) {
            try {
                ...
                server_logger->error("http_thread stop failed!");
            }
            try {
                http_thread_.join();
            } catch (...) {
                server_logger->error("http_thread join failed!");
            }
        }
...
bool GatewayServer::init_server(uint16_t ws_port, uint16_t http_port, const std::string& log_path) {
    try {
        ...
        std::cerr << "Failed to initialize GatewayServer: " << e.what() << std::endl;
        return false;
    } catch (...) {
        if (server_logger) {
            server_logger->error("Unknown exception during GatewayServer initialization");
        }
        std::cerr << "Unknown exception during GatewayServer initialization" << std::endl;
        return false;
    }
}
...
void GatewayServer::init_ws_server(uint16_t port) {
    try {
        ...
        try {
            ...
            ConfigManager config(config_path_);
            auto cert_path = config.get<std::string>("gateway.cert_file",
                                                     "test/network/test_cert.pem");
            auto key_path = config.get<std::string>("gateway.key_file",
                                                    "test/network/test_key.pem");
            if (!std::filesystem::exists(cert_path) || !std::filesystem::exists(key_path)) {
                throw std::runtime_error("TLS certificate or private key file does not exist");
            }

            server_logger->info("Using WebSocket TLS cert: {} and key: {}", cert_path, key_path);
            ssl_ctx_.use_certificate_chain_file(cert_path);
            ssl_ctx_.use_private_key_file(key_path, boost::asio::ssl::context::pem);
        ...
        server_logger->error("Failed to start websocket server: {}", e.what());
        throw std::runtime_error("Failed to start websocket server: " + std::string(e.what()));
    } catch (...) {
        server_logger->error("Failed to start websocket server: unknown exception");
        throw std::runtime_error("Failed to start websocket server: unknown exception");
    }
}
/**
 * @brief 初始化HTTP服务器
