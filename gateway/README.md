...
The Gateway is the client-facing process. It owns HTTP/WebSocket protocol
adapters, online connection state, authentication integration, command handler
registration, and local/remote service adapter composition.

## Directory Map

- `gateway_server/` - process-level composition. `GatewayServer` initializes
  HTTP, WebSocket, auth, ODB-backed local service adapters, remote gRPC
  adapters, command handlers, and route configuration.
- `command_handlers/` - command registration and HTTP/WS business entrypoints.
  Handlers consume `UnifiedMessage`, verify the token context when needed,
  call service adapters, and return `ProcessorResult`.
- `http/` - HTTP protocol helpers only.
- `service_adapters/` - service adapter interfaces and remote gRPC adapter
  implementations for User, Message, Friend, and Group.
...
- `router/` - configurable HTTP path/method to command mapping used before
...
- `IM_ENABLE_USER_HTTP` - user command handlers and service adapter.
- `IM_ENABLE_MESSAGE_HTTP` - message command handlers and service adapter.
...
- `IM_ENABLE_FRIEND_HTTP` - friend command handlers and service adapter.
- `IM_ENABLE_GROUP_HTTP` - group command handlers and service adapter.
- `IM_ENABLE_GROUP_MESSAGE_HTTP` - group-message command handlers and push fanout.
...
- HTTP command handlers do not trust client-supplied sender UID; the access
  token is the actor identity.
