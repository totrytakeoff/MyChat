...
struct UserServerRuntimeConfig {
...
UserServerRuntimeConfig g_config;
im::service::user::UserServerApp* g_server = nullptr;

std::string build_pg_connection_string(const UserServerRuntimeConfig& config) {
...
void configure_cli(im::utils::CLIParser& parser) {
    parser.addArgument("config", 'c', im::utils::ArgumentType::STRING, false,
                       "User server config file", g_config.config_file, "Configuration",
                       [](const std::string& value) {
                           g_config.config_file = value;
                           return true;
                       });
    parser.addArgument("listen", 'L', im::utils::ArgumentType::STRING, false,
                       "gRPC listen address, e.g. 0.0.0.0:9001", "", "Network",
                       [](const std::string& value) {
...
void shutdown_user_server(int, const std::string&) {
    if (g_server) {
        g_server->shutdown();
    }
}
...
int main(int argc, char** argv) {
    try {
        im::utils::CLIParser parser("user_server", "MyChat User gRPC server");
        ...
        g_config.listen_address = config.getWithEnv<std::string>(
            "user.listen_address", "MYCHAT_USER_LISTEN_ADDRESS", g_config.listen_address);
        ...
        g_config.log_level = config.getWithEnv<std::string>(
            "user.log_level", "MYCHAT_LOG_LEVEL", g_config.log_level);

        im::utils::LogManager::SetLogLevel(g_config.log_level);
        if (!im::utils::ServiceIdentityManager::getInstance().initializeFromEnv("user")) {
            std::cerr << "Failed to initialize user service identity" << std::endl;
            return 1;
        }

        im::service::user::UserServerConfig server_config;
        ...
        im::service::user::UserServerApp server(server_config);
        g_server = &server;

        auto& signal_handler = im::utils::SignalHandler::getInstance();
        signal_handler.registerGracefulShutdown(shutdown_user_server);

        if (!server.start()) {
            return 1;
        }

        std::cout << "User server started. gRPC listen address: "
                  << server.listen_address() << std::endl;
        signal_handler.waitForShutdown("Press Ctrl+C to shutdown user_server...");
        signal_handler.cleanup();
        server.shutdown();
        g_server = nullptr;
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "user_server failed: " << e.what() << std::endl;
