...
struct MessageServerRuntimeConfig {
...
MessageServerRuntimeConfig g_config;
im::service::message::MessageServerApp* g_server = nullptr;

std::string build_pg_connection_string(const MessageServerRuntimeConfig& config) {
...
void configure_cli(im::utils::CLIParser& parser) {
    parser.addArgument("config", 'c', im::utils::ArgumentType::STRING, false,
                       "Message server config file", g_config.config_file, "Configuration",
                       [](const std::string& value) {
                           g_config.config_file = value;
                           return true;
                       });
    parser.addArgument("listen", 'L', im::utils::ArgumentType::STRING, false,
                       "gRPC listen address, e.g. 0.0.0.0:9002", "", "Network",
                       [](const std::string& value) {
...
void shutdown_message_server(int, const std::string&) {
    if (g_server) {
        g_server->shutdown();
    }
}
...
int main(int argc, char** argv) {
    try {
        im::utils::CLIParser parser("message_server", "MyChat Message gRPC server");
        ...
        g_config.listen_address = config.getWithEnv<std::string>(
            "message.listen_address", "MYCHAT_MESSAGE_LISTEN_ADDRESS", g_config.listen_address);
        ...
        g_config.log_level = config.getWithEnv<std::string>(
            "message.log_level", "MYCHAT_LOG_LEVEL", g_config.log_level);

        im::utils::LogManager::SetLogLevel(g_config.log_level);
        if (!im::utils::ServiceIdentityManager::getInstance().initializeFromEnv("message")) {
            std::cerr << "Failed to initialize message service identity" << std::endl;
            return 1;
        }

        im::service::message::MessageServerConfig server_config;
        ...
        im::service::message::MessageServerApp server(server_config);
        g_server = &server;

        auto& signal_handler = im::utils::SignalHandler::getInstance();
        signal_handler.registerGracefulShutdown(shutdown_message_server);

        if (!server.start()) {
            return 1;
        }

        std::cout << "Message server started. gRPC listen address: "
                  << server.listen_address() << std::endl;
        signal_handler.waitForShutdown("Press Ctrl+C to shutdown message_server...");
        signal_handler.cleanup();
        server.shutdown();
        g_server = nullptr;
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "message_server failed: " << e.what() << std::endl;
