...
struct GroupServerRuntimeConfig {
...
GroupServerRuntimeConfig g_config;
im::service::group::GroupServerApp* g_server = nullptr;

std::string build_pg_connection_string(const GroupServerRuntimeConfig& config) {
...
void configure_cli(im::utils::CLIParser& parser) {
    parser.addArgument("config", 'c', im::utils::ArgumentType::STRING, false,
                       "Group server config file", g_config.config_file, "Configuration",
                       [](const std::string& value) {
                           g_config.config_file = value;
                           return true;
                       });
    parser.addArgument("listen", 'L', im::utils::ArgumentType::STRING, false,
                       "gRPC listen address, e.g. 0.0.0.0:9004", "", "Network",
                       [](const std::string& value) {
...
void shutdown_group_server(int, const std::string&) {
    if (g_server) {
        g_server->shutdown();
    }
}
...
int main(int argc, char** argv) {
    try {
        im::utils::CLIParser parser("group_server", "MyChat Group gRPC server");
        ...
        g_config.listen_address = config.getWithEnv<std::string>(
            "group.listen_address", "MYCHAT_GROUP_LISTEN_ADDRESS", g_config.listen_address);
        ...
        g_config.log_level = config.getWithEnv<std::string>(
            "group.log_level", "MYCHAT_LOG_LEVEL", g_config.log_level);

        im::utils::LogManager::SetLogLevel(g_config.log_level);
        if (!im::utils::ServiceIdentityManager::getInstance().initializeFromEnv("group")) {
            std::cerr << "Failed to initialize group service identity" << std::endl;
            return 1;
        }

        im::service::group::GroupServerConfig server_config;
        ...
        im::service::group::GroupServerApp server(server_config);
        g_server = &server;

        auto& signal_handler = im::utils::SignalHandler::getInstance();
        signal_handler.registerGracefulShutdown(shutdown_group_server);

        if (!server.start()) {
            return 1;
        }

        std::cout << "Group server started. gRPC listen address: "
                  << server.listen_address() << std::endl;
        signal_handler.waitForShutdown("Press Ctrl+C to shutdown group_server...");
        signal_handler.cleanup();
        server.shutdown();
        g_server = nullptr;
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "group_server failed: " << e.what() << std::endl;
