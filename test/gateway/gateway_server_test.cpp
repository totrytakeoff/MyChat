...
protected:
    void SetUp() override {
        ...
        im::db::RedisManager::GetInstance().initialize(redis_config);
    }
...
TEST_F(GatewayServerTest, ConstructorTest) {
    EXPECT_NO_THROW({
        gateway_server = std::make_unique<GatewayServer>(platform_config_path, router_config_path,
                                                         ws_port, http_port);
    });
...
TEST_F(GatewayServerTest, StartStopTest) {
    cout << "GatewayServerTest::StartStopTest" << endl;

    gateway_server = std::make_unique<GatewayServer>(platform_config_path, router_config_path,
                                                     ws_port, http_port);

    cout << "gateway_server->start()" << endl;
    // 启动服务器
    EXPECT_NO_THROW({ gateway_server->start(); });

    cout << "gateway_server->is_running()" << endl;
    EXPECT_TRUE(gateway_server->is_running());

    cout << "gateway_server->stop()" << endl;
    // 停止服务器
    EXPECT_NO_THROW({ gateway_server->stop(); });

    cout << "gateway_server->is_running()" << endl;
    EXPECT_FALSE(gateway_server->is_running());
    cout << "GatewayServerTest::StartStopTest end" << endl;
}
...
TEST_F(GatewayServerTest, RegisterMessageHandlersTest) {
    ...
    // };
    auto test_handler = [&handler_called](const UnifiedMessage& msg) -> ProcessorResult {
        handler_called = true;
        return ProcessorResult(0, "Success");
    };


    // 注册处理函数
    bool result = gateway_server->register_message_handlers(2003, test_handler);
    EXPECT_TRUE(result);

    // 尝试重复注册同一个cmd_id，应该失败
    result = gateway_server->register_message_handlers(2003, test_handler);
    EXPECT_FALSE(result);

    // 尝试注册另一个cmd_id，应该成功
    result = gateway_server->register_message_handlers(2004, test_handler);
    EXPECT_TRUE(result);
}
...
TEST_F(GatewayServerTest, PerformanceStartStopTest) {
    ...
    cout << "GatewayServerTest::PerformanceStartStopTest  stop" << endl;
    gateway_server->stop();
    cout << "GatewayServerTest::PerformanceStartStopTest  stop" << endl;
    ...
    auto init_duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(init_time - start_time);
    auto start_duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(start_time_end - init_time);
    auto stop_duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(stop_time - start_time_end);
...
TEST_F(GatewayServerTest, PerformanceMessageProcessingTest) {
    ...
    //     co_return CoroProcessorResult(0, "Success");
    // };
    auto test_handler = [&message_count](const UnifiedMessage& msg) -> ProcessorResult {
        message_count.fetch_add(1);
        // 模拟处理延迟
        std::this_thread::sleep_for(std::chrono::microseconds(100));
        return ProcessorResult(0, "Success");
...
TEST_F(GatewayServerTest, ExceptionHandlingTest) {
    // 测试使用无效配置文件路径
    // 当前实现构造函数内部捕获错误并记录日志，不抛出异常
    EXPECT_NO_THROW({ GatewayServer invalid_server("invalid_config.json", "invalid_router.json", 8092, 8093); });
}
...
TEST_F(GatewayServerTest, MessageHandlerInvocationTest) {
    ...
    // auto test_handler =
    //         [&handler_call_count](const UnifiedMessage& msg) -> Task<CoroProcessorResult> {
    //     handler_call_count.fetch_add(1);
    //     co_return CoroProcessorResult(0, "Success");
    // };
    auto test_handler = [&handler_call_count](const UnifiedMessage& msg) -> ProcessorResult {
        handler_call_count.fetch_add(1);
        return ProcessorResult(0, "Success");
    };

    // 注册处理函数
    bool result = gateway_server->register_message_handlers(2005, test_handler);
    EXPECT_TRUE(result);
